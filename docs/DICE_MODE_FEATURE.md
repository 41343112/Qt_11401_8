...
骰子模式是線上多人對戰專屬的遊戲變體模式，為遊戲增加了隨機性和策略深度。在此模式下，玩家每個回合獲得3個隨機選定的棋子，**必須使用完所有3個骰子棋子後才能換對手**，這些棋子都是當前有合法移動的棋子。

## 功能特色 (Features)

### 1. 隨機棋子選擇 (Random Piece Selection)
- 每個玩家回合開始時，系統會自動投擲3個骰子
...
### 3. 三步回合制 (3-Move Turn System)
- 玩家每個回合必須**移動所有3個骰子棋子**
- 在3個骰子都使用完之前，回合不會切換到對手
- 使用的骰子會變成灰色，提示還剩幾個骰子可用
- 當所有3個骰子都用完後，才輪到對手
...
- 當一個玩家使用完所有3個骰子後，回合切換到對手
- 對手回合開始時，自動為對手投擲新的3個骰子
- 新的骰子只包含當前玩家可合法移動的棋子
...
3. **玩家完成移動後**
   - 標記使用的骰子為灰色
   - **檢查是否還有未使用的骰子**
   - 如果還有骰子未使用，**強制保持當前玩家回合**（不切換）
   - 如果所有3個骰子都已使用，允許回合切換到對手
   - 發送移動給對手

4. **對手完成移動後**
   - 當對手使用完所有3個骰子，輪到本地玩家時
   - 自動為本地玩家投擲新的3個骰子
   - 更新骰子顯示
...
2. **完成3步移動**：必須移動所有3個骰子棋子
3. **第一步**：選擇並移動骰子#1 → 變成灰色
4. **第二步**：選擇並移動骰子#2 → 變成灰色
5. **第三步**：選擇並移動骰子#3 → 變成灰色
6. **回合結束**：所有骰子用完後，輪到對手
7. **對手回合**：對手獲得新的3個骰子，重複上述流程

## 策略提示 (Strategy Tips)

1. **計劃3步走法**：由於必須完成所有3個骰子，需要提前規劃整個回合
2. **適應隨機性**：棋子是隨機的，要靈活調整策略
3. **保持選擇多樣性**：盡量讓多個棋子保持可移動狀態
4. **預測對手**：對手也受到骰子限制，可以根據其棋子分佈猜測可能的移動
5. **緊急應變**：即使在將軍情況下，骰子也只顯示可解除將軍的棋子
6. **連續攻擊**：利用3步移動的優勢，可以執行連續進攻策略
...
5. **三步回合制**：每個玩家必須連續移動3個棋子，與標準西洋棋規則不同
